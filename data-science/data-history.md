...
动物本身也能传递一定的信息，例如通过叫声或者一些简单的动作等，人类的早期也是一样，并没有什么数字或者文字，或许也不需要。不过聪明也幸运的人类祖先们，逐步演化出了自己的文字系统，这样文明的演化才有了基础。

我们小时候，大人们教我们识字，大都是从手指计数开始的吧，123...789就这样数着，数到10就没法数了，加上脚趾，最多数到20吧。于是大人们就开始教我们数石子，石子的数量可比手指的数量多多了。手指和石子都可以认为是一种计数系统，不过这样的计数系统有很大的局限。这些问题古人可能也会遇到，而我们聪明的先人想到在绳子上打结。

据文献记载及考古发现，人类的结绳记事约起源自新石器时代，历经漫长的传承，一直延续到上个世纪，并遍布世界范围，中国、日本、波斯、埃及、墨西哥、秘鲁、波利尼西亚等地均曾盛行。在我国，藏族、蒙古族、傈僳族、独龙族、哈尼族、德昂族、怒族、佤族、高山族、基诺族、苗族、黎族、普米族、瑶族等少数民族也都有结绳的历史。

最简单的结绳用一个结表示1，两个结表示2...；进阶一点，可以用绳结的大小或位置来表示不同的数位。这其实有点类似我们小时候用的算盘，它并不是每个算子都表示1，而是有进位的概念：有些表示1，有些表示5......其实如果使用进位的概念，我们的手指也能表示很大的数据。
对于结绳计数，这还是比较好理解的。聪明的先人们可不满足于只是计数，这里有两个故事：

- 东汉末年儒家学者、经学大师郑玄在《周易郑康成注》一书中有言：「事大，大结其绳；事小，小结其绳。」
- 1937年《贵州苗民概况》一书中提到：「苗人识字甚少，犹保持上古结绳记事遗风。遇事暗中以草记之，简单实例，日久尚能忆之。」这听着就像谍战片的场景。

看还可以结绳记事，此外心灵手巧的人们还发展出来了结绳通讯等。如果不是结绳本身具有很大的局限性，说不定人们能发展出一套“结绳计算机”。

后来事情越来越复杂，结绳越来越不能满足生活与生产的需要，于是我们伟大的文字就慢慢演化出来了。

## 数据的今生
如果我们使用手指计数，那手指就是我们存储信息的载体（当然这种载体很不稳定，因为没人可以将手指一直保持一个姿势）；如果我们使用石子计数，那石子也是我们的载体（但是这种载体很沉）；如果我们结绳计数，那么结绳就是我们存储的载体。当然，我们的先人们还发明了算盘这种当时应该是最先进的“计算机”。那我们把文字写在纸上，那“纸”是不是也是我们的所理解的存储载体呢？

我觉得这说法，对也不对。对是因为文字确实是在纸上，而不对呢是因为这和我们说的载体并不一样。如果我们只是理解文字写在了纸上，这并不能有助于我们理解我们是怎么解读到纸上的信息的。事实上，当我们在一张白纸上写下一个黑字的时候，我们应该这样子理解：
白纸是一个n×m的矩阵，矩阵元素全是0，当我们写上一个字的时候，矩阵中的某些元素就变成了1，也就是黑色的部分，我们的眼睛看到了这些变化（输入），而我们大脑的神经网络就开始运算识别，最后输出我们写的（矩阵）是什么字。

事实上，我们计算机上显示的文字就是一个个的矩阵。我们看到的是一张纸，但实际上，那矩阵上一个个的点才是我们理解的信息的载体。

### 1. 开关与位
历史很快就发展到了近代，我们有了电，有了电路和开关，然后还有了电子管，晶体管，这就是现在计算机存储的基础。晶体管可以简单理解就是开关集合，一个开关自然有开和关两种状态，对应着就表示0和1，这是存储的最小单位，也就是**位**，英文是“bit”。

三体里有一大段是用来描述一个人列计算机，几千万人整齐的站在一起，通过举白旗或者黑旗来表示0或1，这样一个人就是一个开关，表示一个“位”。其实，这个对比人类结绳的故事也是很类似的。
