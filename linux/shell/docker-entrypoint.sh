#!/bin/bash
# https://www.cnblogs.com/didispace/p/14196932.html
# https://www.cnblogs.com/breezey/p/8812197.html
# set -e: 一旦出现非0的返回值立即退出
# set -o pipefail: 管道默认只返回最后一个命令的返回值，加上这个之后会返回第一个非零值
set -eo pipefail
...
if [ "$1" = 'mysqld' -a -z "$wantHelp" -a "$(id -u)" = '0' ]; then
	...
	# gosu 是一个 golang 语言开发的工具, 用来取代 shell 中的 sudo 命令. 
	# su 和 sudo 命令有一些缺陷, 主要是会引起不确定的 TTY, 对信号量的转发也存在问题. 
	# 如果仅仅为了使用特定的用户运行程序, 使用 su 或 sudo 显得太重了, 为此 gosu 应运而生.
	# gosu 首先找出指定的用户或用户组, 然后切换到该用户或用户组. 接下来, 使用 exec 启动应用程序.
...
# 当实际使用镜像的人执行了你没有预料到的可执行命令时, 将会走到脚本的这最后一行, 去执行用户新的可执行命令
exec "$@"
